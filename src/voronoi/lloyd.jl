function lloyd(points::DMT, Ω::F; SMT=SparseMatrixCSC{Int,Int}, tol=1e-3, max_iters=1000, μ=0.0) where {DMT,F}
    n = size(points, 2)
    z₀ = vec(points)
    ...
    optimal_points = reshape(optimize_voronoi(f, g!, c!, Ω, z₀, 3n, n, μ; tol=tol, max_iters=max_iters), 3, :)

    return bounded_voronoi(optimal_points, Ω)
end


function eval_cvt_objective(z, voronoi)
    points = reshape(z, 3, :)
