function centroidal_voronoi_vox(points::DMT, Ω::F, domain, sqdist; tol=1e-3, max_iters=1000) where {DMT,F}
    ...
    o = BFGSOptimizerState(f, g!, Ω, length(z₀), size(points, 2); f_abs_tol=1e-8, μ₀=5e-3, ST=Float32, VT=CuVector{Float32}, MT=CuMatrix{Float32}, max_iters=max_iters)
